...
{
    SYSCTL_RCGCUART_REG |= (1<<2); //TURN UART CLOCK
    while (!(SYSCTL_PRUART_REG & 0X04)); // MAKE SURE UART CLOCK IS STABLE
...
{
    SYSCTL_REGCGC2_REG |= (1<<3); //TURN PORTA CLOCK
    while (!(SYSCTL_PRGPIO_REG & 0X08)); // MAKE SURE PORTA CLOCK IS STABLE
